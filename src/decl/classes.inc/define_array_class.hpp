	...
	return lock_or_throw_linkage_error(
		...
		[&] -> expected<c&, reference> {
			auto declared_instance_fields 
				= posix::allocate<instance_field, uint16, declared_instance_field_index>(2);
			...
			return emplace_back(
				...
				object_class.get(),
				posix::memory<c*>{},
				posix::memory<static_field, uint16, declared_static_field_index>{},
				move(declared_instance_fields),
				posix::memory<static_method, uint16, declared_static_method_index>{},
				posix::memory<instance_method, uint16, declared_instance_method_index>{},
				optional<method>{},
				is_array_class{ true },
